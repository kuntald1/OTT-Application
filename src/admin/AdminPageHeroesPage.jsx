...
import { Image as ImageIcon, Video as VideoIcon, Type, Clapperboard } from "lucide-react";
import { fetchAdminPageHeroes, updateAdminPageHero } from "./adminApi";
...
export default function AdminPageHeroesPage() {
  ...
  useEffect(() => {
    const existing = heroesByKey[pageKey];
    setForm({
  ...
  const handleSave = async () => {
    ...
    setSaved(false);
    try {
      const updated = await updateAdminPageHero(pageKey, {
        contentType: form.content_type, eyebrow: form.eyebrow, headline: form.headline, subtext: form.subtext, mediaFile,
      });
      ...
      setError(err.message || "Couldn't save the hero.");
    } finally {
      setSaving(false);
    }
  };

  const currentMediaUrl = heroesByKey[pageKey]?.media_url;
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="max-w-xl rounded-xl p-5" style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(245,235,221,0.1)" }}>
          ...
          {(form.content_type === "image" || form.content_type === "video") && (
            <div className="mb-4">
              <label style={labelStyle}>
                {form.content_type === "image" ? "Upload Image" : "Upload Video"}
              </label>
              {currentMediaUrl && !mediaFile && (
                <div className="mb-2">
                  {heroesByKey[pageKey]?.content_type === "image" ? (
                    <img src={currentMediaUrl} alt="" className="h-24 w-40 rounded-lg object-cover" style={{ border: "1px solid rgba(245,235,221,0.15)" }} />
                  ) : (
                    <video src={currentMediaUrl} className="h-24 w-40 rounded-lg object-cover" style={{ border: "1px solid rgba(245,235,221,0.15)" }} muted />
                  )}
                  <p className="mt-1 text-[11px]" style={{ color: "rgba(245,235,221,0.4)" }}>Current — choose a file below to replace it.</p>
                </div>
              )}
              <input
                type="file"
                accept={form.content_type === "image" ? "image/jpeg,image/png,image/webp" : "video/mp4,video/webm,video/quicktime"}
                onChange={(e) => setMediaFile(e.target.files?.[0] || null)}
                className="w-full text-xs"
                style={{ color: "rgba(245,235,221,0.7)" }}
              />
            </div>
          )}
